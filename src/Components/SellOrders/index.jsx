...
import { useSellOrder } from "../../Providers/SellOrderContext";

const SellOrders = () => {
  const {user} = useUser();
  const {sellOrders, setSellOrders, createSellOrder, createOrderError, updateSellOrder, removeSellOrder, getSellOrder, getSellOrders} = useSellOrder();
  ...
  const handleSelect = (val) => {
    ...
    if (!select.includes(val)) {
      ...
      if (val === "all") {
        const selected = ["all"];
        sellOrders.map((order) => selected.push(order.id));
        setSelect([...selected]);
      }
    }
  };
  ...
  const handleSearchChange = async(e) => {
    ...
    setSearch(value);
    if (value) {
      const filteredData = sellOrders.filter(
        (order) =>
  ...
  const handleEdit = () => {
    ...
    const selectedItem = sellOrders.filter((order) => order._id === selectedID);
    setEdit({
  ...
  const handleEditSubmit = async() => {
    const selectedID = select.includes("all") ? select[1] : select[0];
    let editedData = {};
    const fetchOrder = await getSellOrder(selectedID);
    if (fetchOrder) {
   ...
   await updateSellOrder(selectedID, editedData);
    setEditModal(false);
  };

  const handleDelete = () => {
    if(select.includes("all")){
      sellOrders.forEach(async(order) => await removeSellOrder(order._id))
    } 
    if(select.length > 0 && !select.includes('all')){
      select.forEach(async(id) => {
        await removeSellOrder(id);
      })
    }
  };
  ...
  useEffect(() => {
    const fetchOrders = async() => {
      if(user && user.email)await getSellOrders()
    }
    fetchOrders()
  }, [user])
        ...
        <ModalContentContainer>
          <DetailsContainer>
            <Content>
              <Property>
                <Label>Status: </Label>
                <Input value={add.status} onChange={(e) => handleChange(e, "status")} />
              </Property>
              <Property>
                <Label>ClientName: </Label>
                <Input
